...
mapping = json.load(open('anchor_map.json')) # It's a dictionary of frame_idx -> tile to anchor mapping, each mapping is a dict of key duck_id and value original_id

duck_frames = {} # It's a dictionary of frame_idx -> anchor, which is a dict with 'id' and 'shape' keys
current_map = None
for frame_idx, tiles in duck_frames.items():
