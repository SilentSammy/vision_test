...
def estimate_square_pose(square_corners, frame, camera_matrix, dist_coeffs, square_length):
    ...
    # Ensure square_corners is reshaped to (4,2).
    img_points = square_corners.reshape(4, 2)
...
try:
    ...
    while sim.getSimulationState() != sim.simulation_stopped:
        ...
        if squares:
            ...
            s = s.reshape(1, 4, 2)
            # s = s[:, :, ::-1]
            pose = estimate_marker_pose(s, frame, camera_matrix=camera_matrix, dist_coeffs=dist_coeffs, marker_length=0.1)
            if pose is not None:
                yaw, pitch, roll, distance, cam_pitch, cam_yaw = pose
                print(f"Pose: Yaw={yaw}, Pitch={pitch}, Roll={roll}, Distance={distance}, Cam_Pitch={cam_pitch}, Cam_Yaw={cam_yaw}")

        # Draw the detected squares
        # for square in squares:
        #     cv2.polylines(frame, [square], isClosed=True, color=(0, 255, 0), thickness=2)
